...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    e.preventDefault()
    if (!username || !email || !password) { enqueueSnackbar('Please fill all fields', { variant: 'warning' }); return }
    if (password.length < 6) { enqueueSnackbar('Password must be at least 6 characters', { variant: 'warning' }); return }
    setLoading(true)
    try {
    ...
    <div className="auth-page">
      <div className="auth-card">
        <Box display="flex" flexDirection="column" alignItems="center" mb={3}>
          ...
          <Typography variant="body2" color="text.secondary" mt={0.5}>Join the community today</Typography>
        </Box>

        <Typography variant="h6" fontWeight={700} mb={2.5}>Create Account ✨</Typography>
        ...
        <Box component="form" onSubmit={handleSignup} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <TextField fullWidth label="Password" type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
            InputProps={{
              ...
              endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPass(!showPass)} edge="end">{showPass ? <VisibilityOffIcon /> : <VisibilityIcon />}</IconButton></InputAdornment>
            }}
            sx={{ '& .MuiOutlinedInput-root': { borderRadius: 3 } }} />
        ...
        <Box mt={3} textAlign="center">
          <Typography variant="body2" color="text.secondary">
            ...
            <Link to="/login" style={{ color: '#1976d2', fontWeight: 700, textDecoration: 'none' }}>Login</Link>
          </Typography>
        </Box>
      </div>
    </div>
